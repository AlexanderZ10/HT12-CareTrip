...
function sanitizeTargetUrl(value: unknown) {
  ...
  throw new HttpsError(
    "invalid-argument",
    "target must be a valid app, https or local development URL."
  );
}
...
export const stripeCheckoutReturnBridge = onRequest(
  { region: "us-central1" },
  async (request, response) => {
    try {
      const targetUrl = sanitizeTargetUrl(request.query.target);
      const checkout =
        sanitizeString(request.query.checkout).toLowerCase() === "cancel" ? "cancel" : "success";
      const kind =
        sanitizeString(request.query.kind).toLowerCase() === "expense-repayment"
          ? "expense-repayment"
          : "booking";
...
<html lang="bg">
  ...
  <body>
    ...
    <script>
      ...
      const message =
        error instanceof HttpsError ? error.message : "Неуспешно връщане към приложението.";
