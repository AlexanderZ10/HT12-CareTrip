...
exports.searchFreeHotels = searchFreeHotels;
function normalizeSearchFreeHotelsInput(input) {
...
function normalizeExactHotelUrl(params) {
    ...
    try {
        ...
        const isGenericBookingSearch = hostname.includes("booking.com") && pathname.includes("/searchresults");
        const isGenericAirbnbSearch = hostname.includes("airbnb.") && pathname.includes("/s/");
...
async function searchMakCorps(input) {
    try {
        ...
        return results
            .filter((item) => item && (item.name || item.hotel_name))
            .slice(0, 6)
            .map((item) => {
...
function buildViewbox(params) {
    const latDelta = params.radiusKm / 111;
    const lonDelta = params.radiusKm / Math.max(1, 111 * Math.cos(toRadians(params.lat)));
    return [
        params.lon - lonDelta,
        params.lat + latDelta,
        params.lon + lonDelta,
...
async function searchGeoapifyHotels(input) {
    try {
        const coords = await geocodeCity(input.destination);
        if (!coords)
            return [];
        const url = `https://nominatim.openstreetmap.org/search?q=hotel&format=jsonv2&limit=12&addressdetails=1&bounded=1&viewbox=${encodeURIComponent(buildViewbox({
            lat: coords.lat,
            lon: coords.lon,
            radiusKm: 25,
        }))}`;
...
// ── Combined search ─────────────────────────────────────────────────────────
async function searchFreeHotels(input) {
