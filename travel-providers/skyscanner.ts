export type TravelDateParts = {
  day: number;
  month: number;
  year: number;
};

export type SkyscannerAutosuggestPlace = {
  cityName: string;
  displayLabel: string;
  entityId: string;
  iataCode: string;
...
export type SkyscannerFlightOffer = {
  bookingUrl: string;
...
export type SkyscannerHotelOffer = {
  area: string;
  bookingUrl: string;
  imageUrl: string;
  name: string;
...
export type SkyscannerSearchContext = {
  adults: number;
  apiKey: string;
...
function getMapEntries(value: unknown) {
...
async function callSkyscanner<TResponse>(
  apiKey: string,
  path: string,
  body: UnknownRecord
) {
...
function buildFlightPlaceId(place: SkyscannerAutosuggestPlace) {
  if (place.entityId) {
...
function toDateParts(value: TravelDateParts) {
  return {
    day: value.day,
    month: value.month,
    year: value.year,
...
export async function autosuggestFlightPlace(params: {
  apiKey: string;
  locale: string;
  market: string;
  searchTerm: string;
}) {
  const response = await callSkyscanner<{ places?: unknown[] }>(
    params.apiKey,
    "/apiservices/v3/autosuggest/flights",
    {
      query: {
...
export async function autosuggestHotelPlace(params: {
  ...
  const response = await callSkyscanner<{ places?: unknown[] }>(
    params.apiKey,
    "/apiservices/v3/autosuggest/hotels",
    {
      query: {
        locale: params.locale,
        market: params.market,
...
function resolveFlightResults(payload: unknown) {
...
function describeLeg(
  ...
  return {
    durationMinutes: legSummaries.reduce(
...
export async function searchSkyscannerFlightOffers(
  params: SkyscannerSearchContext
) {
  ...
  const createResponse = await callSkyscanner<{
    content?: unknown;
    sessionToken?: string;
    status?: string;
  }>(
  ...
  let latestPayload: unknown = createResponse;
  ...
  for (let index = 0; index < 3 && status !== "RESULT_STATUS_COMPLETE" && status !== "completed"; index += 1) {
    ...
    const pollResponse = await callSkyscanner<unknown>(
      params.apiKey,
      `/apiservices/v3/flights/live/search/poll/${sessionToken}`,
      {}
    );
  ...
  return getMapEntries(itineraries)
    ...
    .map((itinerary) => {
      ...
      const firstPricingItem = asRecord(
        Array.isArray(firstPricingOption.items) ? firstPricingOption.items[0] : undefined
      );
      const firstAgentId =
        asString(firstPricingItem.agentId) ||
...
function resolveHotelResults(payload: unknown) {
...
export async function searchSkyscannerHotelOffers(
  params: SkyscannerSearchContext
) {
  ...
  const createResponse = await callSkyscanner<{
    ...
    params.apiKey,
    "/apiservices/v1/hotels/live/search/create",
    {
  ...
  for (let index = 0; index < 4 && status !== "completed"; index += 1) {
    ...
    const pollResponse = await callSkyscanner<unknown>(
      params.apiKey,
      `/apiservices/v1/hotels/live/search/poll/${sessionToken}`,
      {
        pageIndex: 0,
  ...
  const { agents, hotelContent, hotelInfo, hotelsPricingOptions } =
    resolveHotelResults(latestPayload);
      ...
      return [
        {
          area: asString(
            ...
            asString(
              distance.value,
              destinationPlace.displayLabel || params.destinationQuery
            )
          ),
          bookingUrl:
            asString(normalizedOption.deeplink) ||
            asString(hotelMeta.deeplink),
          imageUrl:
            asString(firstImage.fullUrl) ||
        ...
        } satisfies SkyscannerHotelOffer,
      ];
    })
    .filter((offer) => offer.bookingUrl || offer.priceAmount !== null)
    .sort((left, right) => (left.priceAmount ?? Number.MAX_SAFE_INTEGER) - (right.priceAmount ?? Number.MAX_SAFE_INTEGER))
