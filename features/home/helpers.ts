...
function getHomeCopy(language: AppLanguage) {
  if (language === "en") {
    return {
      ...
      initialAssistant: (profileName: string) =>
        `Hi, ${profileName}! I'm your travel assistant. I'll ask you a few questions to plan the perfect trip. Let's start — where do you want to go?`,
      planning: "Planning",
  ...
  if (language === "de") {
    return {
      ...
      generate: "Vorschl\u00E4ge",
      initialAssistant: (profileName: string) =>
        `Hallo, ${profileName}! Ich bin dein Reiseassistent. Ich stelle dir ein paar Fragen, um die perfekte Reise zu planen. Los geht's — wohin m\u00F6chtest du fahren?`,
      planning: "Planung",
  ...
  if (language === "es") {
    return {
      ...
      initialAssistant: (profileName: string) =>
        `\u00A1Hola, ${profileName}! Soy tu asistente de viajes. Te har\u00E9 unas preguntas para planificar el viaje perfecto. Empecemos — \u00BFad\u00F3nde quieres ir?`,
  ...
  if (language === "fr") {
    return {
      ...
      generate: "Suggestions",
      initialAssistant: (profileName: string) =>
        `Salut, ${profileName} ! Je suis ton assistant voyage. Je vais te poser quelques questions pour planifier le voyage parfait. On commence — o\u00F9 veux-tu aller ?`,
  ...
  return {
    ...
    initialAssistant: (profileName: string) =>
      `Здравей, ${profileName}! Аз съм твоят travel асистент. Ще ти задам няколко въпроса, за да планираме перфектното пътуване. Да започнем — накъде искаш да отидеш?`,
    planning: "Планиране",
